...
ip_coords = np.array([line.split() for line in ip_lines], dtype=float)
...
# Get field variable values at the coordinates using 
# nearest neighbor interpolation (order=0).
# Values outside the data are set to 0.
ip_theta = ndimage.map_coordinates(theta, ip_data_coords.T,
                                   order=0, cval=np.nan)

ip_phi = ndimage.map_coordinates(phi, ip_data_coords.T,
                                 order=0, cval=np.nan)

ip_FVF = ndimage.map_coordinates(FVF, ip_data_coords.T,
                                 order=0, cval=np.nan)
