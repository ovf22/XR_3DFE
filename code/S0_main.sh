...
if [ "$Env_installed" = "no" ]; then
	...
	conda install pip
	~/.conda/envs/STenv/bin/pip install numpy matplotlib nibabel structure-tensor jupyter pandas scipy
...
for sample_name in ${sample_names[@]}; do
	...
	unset SLURM_GTIDS
	abq2023 cae noGUI="${sample_name}_S2_Cube.py"
	wait
	...
	sed "s/InputOrientFortran1/$sample_name\_PHI/;s/InputOrientFortran2/$sample_name\_THETA/;s/InputOrientFortran3/$sample_name\_FVF/" I2_user_subs.f > "${sample_name}_user_subs.f"
	sed "s/InputModelCase/$sample_name/" S4_Cube_modified.py > "${sample_name}_S4_Cube_modified.py"
	abq2023 cae noGUI="${sample_name}_S4_Cube_modified.py"
