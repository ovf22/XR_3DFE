...
def Int_point_plotting(ip_coords, ip_data_set, Nplots, vmin=[-10], vmax=[10],
                       unit='unit', variable='Var', fig_name='Test',
                       fig_path='', fig_title='',
                       figsize=(9, 6), sp_title=None):
    """ 3D scatter plot of FE-model integration points
    with field variable color plot.

    Parameters
    ----------
    ip_coords : Integration point coordinates [Array]\n
    ip_dataset: Data to be visualised at the integration points [Array]\n
    Nplots : Number of plots in subplots [int] \n
    vmin: Lower bound for ip_dataset [float]. Default is -10\n
    vmax: Upper bound for ip_dataset [float]. Default is 10\n
    unit: Field variable unit of data visualised
    at the integration points [str]\n
    variable: Field variable of data visualised
    at the integration points [str]\n
    fig_name: Figure name used for the saved filename [str]\n
    fig_path: Directory of saved figure [str]. Default is working directory\n
    fig_title: Figure title [str]\n
    fig_size: Figure size [tuple]. Default is (9,6)\n
    sp_title: Subplot titles [Array of str]. \n
    ...
    for i in range(Nplots):
        ...
        ax.plot3D(frame_xy[0, :], frame_xy[2, :], frame_xy[1, :],
                  'k', linewidth=0.5, zorder=100)
        ax.plot3D(frame_xz[0, :], frame_xz[2, :], frame_xz[1, :],
                  'k', linewidth=0.5, zorder=100)
        ax.plot3D(frame_yz[0, :], frame_yz[2, :], frame_yz[1, :],
                  'k', linewidth=0.5, zorder=100)
        ...
        clb = fig.colorbar(sc3D, shrink=0.5, pad=0.1,
